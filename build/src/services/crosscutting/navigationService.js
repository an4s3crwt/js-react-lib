...
export class NavigationService extends Service {
  ...
  onNavigationRequest = (contextKey, callbackHandler) => {
    ...
    this.logger.debug(`'${Object.entries(this.navigationRequestSubscriberDictionary).length}' subscribers on 'NavigationRequest'.`);
    return registerKey;
  };
  offNavigationRequest = registerKey => {
    const existingSubscriber = Object.entries(this.navigationRequestSubscriberDictionary).find(([key, value]) => key === registerKey);
    if (existingSubscriber) {
      delete this.navigationRequestSubscriberDictionary[registerKey];
      this.logger.debug(`Component with key '${registerKey}' has unsubscribed from 'NavigationRequest'.`);
      this.logger.debug(`'${Object.entries(this.navigationRequestSubscriberDictionary).length}' subscribers on 'NavigationRequest'.`);
      return true;
    } else {
      this.logger.error(`Component with key '${registerKey}' not found in 'NavigationRequest' subscriptions.`);
      this.logger.debug(`'${Object.entries(this.navigationRequestSubscriberDictionary).length}' subscribers on 'NavigationRequest'.`);
      return false;
    }
  };
