...
export class RESTService extends Service {
  ...
  invokeAsync = async (method, url, data, init) => {
    ...
    const requestInit = this.getRequestInit(method, { ...init, body: data });
    ...
    try {
      ...
      return {
        ...
        messageStack: [{ logText: `Request failed: ${error.message}` }],
      };
    }
  };
  ...
  processResponse = (responseObject, responseStatus, responseStatusText) => {
    ...
    if (!responseObject) {
      ...
      responseData =
        ...
        responseObject.messageStack &&
        responseObject.payload
          ? responseObject
          : { state: ResponseStateEnumeration.OK, payload: responseObject };
    } else {
      responseData.messageStack.push({
        display: {
          keyNamespace: LocalizationNamespaces.System,
